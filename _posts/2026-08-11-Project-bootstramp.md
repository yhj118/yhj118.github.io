...
- 부트스트랩 다운로드 ~ 적용하기.  
  1) CSS와 JS는 정적 파일이므로 관리할 폴더를 생성.  
    경로는 src/main/resources/static에 tabler 폴더와 하위로 css & js 폴더를 생성한다.  
    static 폴더 바로 아래에 있는 css, js, image 폴더는 부트스트랩인 tabler가 아닌 개발하면서 별도로 작성한 소스를 관리하는 곳이며 이것도 생성한다.  
